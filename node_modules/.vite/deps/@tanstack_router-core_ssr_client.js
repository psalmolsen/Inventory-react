...
async function hydrate(router) {
  ...
  if (serializationAdapters == null ? void 0 : serializationAdapters.length) {
  ...
  router.ssr = { manifest };
  const nonce = (_a = document.querySelector('meta[property="csp-nonce"]')) == null ? void 0 : _a.content;
  ...
  await Promise.all(activeMatches.map(async (match) => {
    ...
    try {
      const route = router.looseRoutesById[match.routeId];
      const parentContext = ((_a2 = activeMatches[match.index - 1]) == null ? void 0 : _a2.context) ?? router.options.context;
      ...
      const headFnContent = await ((_c2 = (_b2 = route.options).head) == null ? void 0 : _c2.call(_b2, assetContext));
      const scripts = await ((_e = (_d = route.options).scripts) == null ? void 0 : _e.call(_d, assetContext));
